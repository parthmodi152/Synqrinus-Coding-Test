...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname main-test) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
;;
;; ***************************************************
;;     Parth Modi
...
;; Examples:
;; (check-expect (formula->tree "=1+2") (make-binode '+ 1 2))
(check-expect
 (formula->tree "=1 + 2 /(1+2)")
 (make-binode '+ 1 (make-binode '/ 2 (make-binode '+ 1 2))))
(check-expect
 (formula->tree "=B2*(B1/B2)")
 (make-binode '* 'B2 (make-binode '/ 'B1 'B2)))
(check-expect (formula->tree "=(A1+6) + 5")
              (make-binode '+ (make-binode '+ 'A1 6) 5))
...
;; Tests
(check-expect (formula->tree "=") 0)
(check-expect (formula->tree "=A1") 'A1)
(check-expect
 (formula->tree "=A1+52*2/(B2 + B3)")
 (make-binode '+ 'A1
              (make-binode '* 52
                           (make-binode '/ 2
                                        (make-binode '+ 'B2 'B3)))))
...
(check-expect (formula->tree "=(B1/10)+A6")
              (make-binode '+ (make-binode '/ 'B1 10) 'A6))
(check-expect (formula->tree "=B1/10+A6")
              (make-binode '/ 'B1 (make-binode '+ 10 'A6)))
(check-expect (formula->tree "=B1/(10+A6)")
              (make-binode '/ 'B1 (make-binode '+ 10 'A6)))
(check-expect
 (formula->tree "=((N2*3)/10) + B9")
 (make-binode '+ (make-binode '/ (make-binode '* 'N2 3) 10) 'B9))
...
;; Examples:
(check-expect (parse-formula "=A2/(A2*B1)")
              (list 'A2 #\/ #\( 'A2 #\* 'B1 #\)))
(check-expect (parse-formula "=A1+(92+H1)*1/(B5+B2)-F2")
              (list 'A1 #\+ #\( 92 #\+ 'H1 #\) #\* 1 #\/ #\(
                    'B5 #\+ 'B2 #\) #\- 'F2))
...
;; Tests:
(check-expect (parse-formula "=C2-A8*(B2/B1)")
              (list 'C2 #\- 'A8 #\* #\( 'B2 #\/ 'B1 #\)))
(check-expect (parse-formula "=") empty)
...
;; Examples:
(check-expect (circular-dependency? dataset-1) true)
...
;; Tests
(check-expect (circular-dependency? dataset-3) false)
(check-expect (circular-dependency? dataset-2) false)
...
(check-expect
 ...
 (list
  ...
  (make-data 'B3 (list 'A1)
             (make-binode '+ (make-var-cell 1 'A1) 3))))
...
;; Tests:
(check-error (eval (make-binode '/ 5 0)) "Division by zero")
(check-expect (eval (make-binode '/ 5 5)) 1)
(check-expect (eval (make-binode '- 5 0)) 5)
(check-expect (eval (make-binode '+ 5 10)) 15)
(check-expect (eval (make-binode '* 5 10)) 50)
...
;; Examples:
(check-expect (convert-cell 1) 1)
(check-expect (convert-cell 'C4) (make-var-cell 1 'C4))
(check-expect (convert-cell '-19C4) (make-var-cell -19 'C4))
...
;; Tests:
(check-expect (convert-cell 'A1) (make-var-cell 1 'A1))
(check-expect (convert-cell '10A5) (make-var-cell 10 'A5))
...
;; Examples:
(check-expect
 (convert-binexp (formula->tree "=8B1/9D5"))
 (make-binode '/
              (make-var-cell 8 'B1)
              (make-var-cell 9 'D5)))
(check-expect
 (convert-binexp (formula->tree "=9B1*(C1)/D5"))
 (make-binode '*
              (make-var-cell 9 'B1)
              (make-binode '/
                           (make-var-cell 1 'C1)
...
;; Tests:
(check-expect
 (convert-binexp (formula->tree "=10A1+B2-C5"))
 (make-binode '+
              (make-var-cell 10 'A1)
              (make-binode '-
                           (make-var-cell 1 'B2)
                           (make-var-cell 1 'C5))))
(check-expect (convert-binexp (formula->tree "=")) 0)
...
(check-expect
 (get-dependencies
  (make-binode '+
               ...
               (make-binode '+
                            (make-var-cell 1 'A1)
                            (make-var-cell 1 'A1)))) (list 'A1))
(check-expect (get-dependencies 0) empty)
(check-expect
 (get-dependencies
  (make-binode
   '+ 1 (make-binode '+ 2
                     (make-binode '* 3
                                  (make-binode '/ 4 5))))) empty)
...
;; Examples:
(check-expect
 (parse-dataset dataset-1)
 (list
  (make-data 'A1 (list 'A2)
             (make-binode '* (make-var-cell 1 'A2) 2))
  (make-data 'A2 (list 'B3)
             (make-var-cell 1 'B3))
  (make-data 'A3 '() 2)
  (make-data 'B1 '() 4)
  (make-data 'B2 '() 3)
  (make-data 'B3 (list 'A1 'B2)
             (make-binode '+
                          (make-var-cell 1 'A1)
                          (make-var-cell 1 'B2)))))
...
;; Tests:
(check-expect
 (parse-dataset dataset-2)
 (list
  (make-data 'A1 '() 3)
  (make-data 'A2 (list 'A1 'B1)
             (make-binode '+ (make-var-cell 1 'A1)
                          (make-var-cell 1 'B1)))
  (make-data 'A3 '() 1)
  (make-data 'B1 (list 'B3) (make-var-cell 1 'B3))
  (make-data 'B2 '() 2)
  (make-data 'B3 '() 3)))

(check-expect
 (parse-dataset dataset-3)
 (list
  (make-data 'A1 (list 'A2 'B1)
             (make-binode '/ (make-var-cell 1 'A2)
                          (make-binode '*
                                       (make-var-cell 1 'A2)
                                       (make-var-cell 1 'B1))))
  (make-data 'A2 (list 'A3 'B2)
             (make-binode '+
                          (make-var-cell 1 'A3)
                          (make-var-cell 1 'B2)))
  (make-data 'A3 '() 3)
  (make-data 'B1 (list 'A2 'B2)
             (make-binode '*
                          (make-var-cell 1 'A2)
                          (make-var-cell 1 'B2)))
  (make-data 'B2 (list 'B3) (make-var-cell 1 'B3))
  (make-data 'B3 '() 1)))
 ...
 (input-values
  (make-binode '+
               ...
               (make-binode '-
                            (make-var-cell 1 'B2)
                            (make-var-cell 1 'C5)))
  (list (make-data 'A1 '() 10)
        (make-data 'B2 '() 15)))
 (make-binode '+ 100 (make-binode '- 15 (make-var-cell 1 'C5))))
(check-expect
 (input-values
  (make-binode '+
               (make-var-cell 10 'A1)
